...
- **Tested Events View**: Inspect the latest stored transactions, their final resolved outcomes, the raw event payload, every rule that fired for each event, see which payload fields those rules reference with hover-based highlighting, jump straight from a trigger to the rule detail page, and refresh the list without reloading the whole app
