...
- **AI Rule Authoring**: Create and edit rules with an inline AI assistant that uses observed fields, field types, user lists, configured outcomes, and lane constraints to generate a draft, explain it line by line, auto-repair obvious validation issues, and require explicit human apply/save approval. Edit-mode drafts can be manually sent to the existing Backtest Results workflow for outcome deltas, label-aware precision/recall/F1, and skipped-record warnings before anything is saved. OpenAI provider/model credentials can be managed per organisation in Settings.
...
- **Analytics Dashboard**: Real-time transaction volume charts, outcome trends, ranked most/least firing active rules, and per-rule performance charts on the Rule Detail page with configurable time ranges (1h, 6h, 12h, 24h, 30d)
...
- **Audit Trail**: Change tracking for rules, user lists, outcomes, labels, field type configurations, API keys, and strict-mode enable/disable actions, with per-change user attribution and explicit rule lifecycle actions (`promoted`, `deactivated`, `rolled_back`, `deleted`) plus neutral-outcome setting changes
- **Strict Mode Control Surface**: Settings now exposes a persisted org-level strict-mode checkbox guarded by `manage_permissions`, and every enable/disable action is written to a dedicated Strict Mode audit section
- **Field Type Management**: Auto-discovers JSON field types from live traffic and test payloads, including canonical dotted nested paths such as `customer.profile.age`; configurable type casting plus optional required/non-null field contracts are applied before rule evaluation so comparisons behave correctly and invalid live events fail fast
...
- **Tested Events View**: Inspect the latest stored transactions, their uploaded labels, final resolved outcomes, the raw event payload, every rule that fired for each event, see referenced payload fields highlighted inline inside the JSON with hover-based rule focus, including nested dotted paths, jump straight from a trigger to the rule detail page, and refresh the list without reloading the whole app
