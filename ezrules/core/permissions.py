...
class PermissionManager:
    ...
    def init_default_actions():
        ...
        for name, description, resource_type in default_actions:
            if not db_session.query(Action).filter_by(name=name).first():
    ...
    def user_has_permission(user, action_name: PermissionAction | str, resource_id: int | None = None) -> bool:
        ...
        actions_exist = db_session.query(Action).first() is not None
        ...
        action = db_session.query(Action).filter_by(name=action_str).first()
        if not action:
            return False

        for role in user.roles:
            role_action = (
                db_session.query(RoleActions)
