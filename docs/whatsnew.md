...
* **Async report snapshots**: Added `POST/GET /api/v2/analytics/rule-quality/reports` to request and poll persisted snapshots. Reports are frozen at a specific `freeze_at` timestamp and reused from cache to avoid recomputing on every page load.
