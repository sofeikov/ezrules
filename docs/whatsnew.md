...
* **Graph feature management UI**: The Features page can now create, edit, display, and activate graph feature definitions with target entity, traversable entity allowlist, depth, and expansion guardrails.
* **Tested Events graph view**: Tested Events rows can now open a bounded event-entity network, fit or reset the layout, and traverse up to three shared-entity event hops by default while respecting configured event limits.
...
* **Current transaction projection**: New versions update `transaction_current_versions` when they become current, keep `first_effective_at` and `first_observed_at` as transaction-level lifecycle facts, mark prior current decisions superseded, and leave late-arriving or post-terminal versions as historical records.
...
* **Legacy testing logs removed from runtime storage**: Evaluation, backtesting, shadow, rollout, CLI demo data, and frontend event identifiers now use the canonical event-version and served-decision ledgers instead of `testing_record_log` / `testing_results_log`.
