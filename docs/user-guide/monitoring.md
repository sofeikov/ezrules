...
Use **Alerts** when an outcome needs active attention instead of passive chart review. Alert rules watch the same canonical served-decision ledger as the dashboard; for example, a rule can notify when `CANCEL` exceeds `50` served decisions in a rolling one-hour window. Alert incidents are recorded and delivered to the in-app notification bell. The unread badge and an open notification list refresh within five seconds while you remain on the current page, and the list scrolls within the viewport when the inbox grows. Opening a spike notification takes you to the Cases queue filtered to that incident; each contributing evaluation keeps the existing case assignment, notes, disposition, and audit workflow. Celery beat also sweeps alert rules on a schedule so missed queue jobs are repaired after worker restarts.
